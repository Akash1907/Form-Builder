...
function App() {
  ...
  const [newComponentField, setNewComponentField] = useState({
    label: "",
    type: "",
    dataType: "",
    options: [],
  ...
  const [generatedComponentSchema, setGeneratedComponentSchema] =
    useState(null);
  const [generatedComponentUiSchema, setGeneratedComponentUiSchema] =
    useState(null);
  ...
  const handleAddComponentField = () => {
    if (newComponentField.label && newComponentField.type) {
      setComponentFields((prev) => [...prev, newComponentField]);
      setNewComponentField({
        label: "",
        type: "",
        dataType: "",
        options: [],
  ...
  const handleGenerateForm = (fields) => {
    ...
    fields.forEach((field) => {
      ...
      if (!childrenSwitch) {
        ...
        setGeneratedComponentSchema(schema);
        setGeneratedComponentUiSchema(uiSchema);
        console.log(
          "ComponentSchema --- ",
    ...
    <Box sx={{ padding: "20px" }}>
      ...
      >
        <Grid item xs={6}>
          ...
          <TextField
            label="Label"
            value={childrenSwitch ? newComponentField.label : newField.label}
            onChange={(e) =>
              childrenSwitch
                ? setNewComponentField((prev) => ({
                    ...prev,
          ...
          <FormControl fullWidth sx={{ marginBottom: "10px" }}>
            ...
            <Select
              value={childrenSwitch ? newComponentField.type : newField.type}
              onChange={(e) =>
                childrenSwitch
                  ? setNewComponentField((prev) => ({
                      ...prev,
          ...
          <Button
            variant="contained"
            color="primary"
            onClick={childrenSwitch ? handleAddComponentField : handleAddField}
            sx={{ marginTop: "10px" }}
          >
            Generate Form
          </Button>
          {componentFields.label}
          {newComponentField.label}
        </Grid>
